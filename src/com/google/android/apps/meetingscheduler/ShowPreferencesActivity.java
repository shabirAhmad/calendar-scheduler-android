...
public class ShowPreferencesActivity extends PreferenceActivity implements
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    timeSpanPref = (ListPreference) getPreferenceScreen().findPreference(timeSpanKey);

    skipWeekendsKey = getString(R.string.skip_weekends_chkbox_pref);
    skipWeekendsPref = (CheckBoxPreference) getPreferenceScreen().findPreference(skipWeekendsKey);
    ...
    workingHoursEndPref = (Preference) getPreferenceScreen().findPreference(workingHoursEndKey);
    workingHoursEndPref.setOnPreferenceClickListener(new OnPreferenceClickListener() {
      @Override
      public boolean onPreferenceClick(Preference preference) {
  ...
  private void setUseWorkingHoursSummary() {
    if (useWorkingHoursPref.isChecked()) {
      useWorkingHoursPref.setSummary(getString(R.string.use_working_hours_summary_checked));
    } else {
      useWorkingHoursPref.setSummary(getString(R.string.use_working_hours_summary_unchecked));
    }
  }
  ...
  private void setWorkingHoursStartSummary() {
    ...
    setTimeSummary(workingHoursStartPref, workingHoursStartHours, workingHoursStartMinutes);
  }
  ...
  private void setTimeSummary(Preference preference, int hourOfDay, int minute) {
    ...
    if (is24HourFormat) {
      time.append(Integer.toString(hourOfDay).length() == 2 ? hourOfDay : "0" + hourOfDay);
      time.append(":");
      time.append(Integer.toString(minute).length() == 2 ? minute : "0" + minute);
    } else {
      int hour = hourOfDay <= 12 ? hourOfDay : hourOfDay - 12;
      time.append(Integer.toString(hour).length() == 2 ? hour : "0" + hour);
      time.append(":");
      time.append(Integer.toString(minute).length() == 2 ? minute : "0" + minute);
  ...
  protected Dialog onCreateDialog(int id) {
    ...
    case WORKING_HOURS_START_ID:
      return new TimePickerDialog(this, workingHoursStartTimeSetListener, workingHoursStartHours,
          workingHoursStartMinutes, is24HourFormat);

    case WORKING_HOURS_END_ID:
      return new TimePickerDialog(this, workingHoursEndTimeSetListener, workingHoursEndHours,
          workingHoursEndMinutes, is24HourFormat);
    }
    return null;
  }
  ...
  protected void onPrepareDialog(int id, Dialog dialog) {
    ...
    switch (id) {
    case WORKING_HOURS_START_ID:
      ((TimePickerDialog) dialog).updateTime(workingHoursStartHours, workingHoursStartMinutes);
      break;
    case WORKING_HOURS_END_ID:
      ((TimePickerDialog) dialog).updateTime(workingHoursEndHours, workingHoursEndMinutes);
    }
  }
