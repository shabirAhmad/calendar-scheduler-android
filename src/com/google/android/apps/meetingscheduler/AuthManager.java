...
public class AuthManager {
   ...
   *          request code
   * @param code The request code to pass to {@link Activity#onActivityResult}
   *          when {@link #authResult(int, Intent)} should be called
   ...
   */
  public AuthManager(Activity activity, int code, Bundle extras, boolean requireGoogle,
      String service) {
   ...
   * 
   * @param runnable A {@link Runnable} to execute when the auth token has been
      ...
      public void run(AccountManagerFuture<Bundle> future) {
        try {
          ...
          if (result.containsKey(AccountManager.KEY_INTENT)) {
            ...
            activity.startActivityForResult(intent, MeetingSchedulerConstants.GET_LOGIN);
            return;
          }

          authToken = result.getString(AccountManager.KEY_AUTHTOKEN);
          Log.e(MeetingSchedulerConstants.TAG, "Got auth token.");
          runWhenFinished();
        } catch (OperationCanceledException e) {
          Log.e(MeetingSchedulerConstants.TAG, "Operation Canceled", e);
        } catch (IOException e) {
          Log.e(MeetingSchedulerConstants.TAG, "IOException", e);
        } catch (AuthenticatorException e) {
          Log.e(MeetingSchedulerConstants.TAG, "Authentication Failed", e);
        }
      }
    }, null /* handler */);
  }
  ...
  public boolean authResult(int resultCode, Intent results) {
    if (results != null) {
      ...
      Log.w(MeetingSchedulerConstants.TAG, "authResult: " + authToken);
    } else {
      Log.e(MeetingSchedulerConstants.TAG, "No auth result results!!");
    }
    runWhenFinished();
   ...
   * 
   * @param runnable A {@link Runnable} to execute when a new auth token is
   *          successfully fetched
   */
  public void doLogin(final Runnable runnable, final boolean invalidate) {
    this.whenFinished = runnable;
